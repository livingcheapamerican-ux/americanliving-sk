...
Deno.serve(async (req) => {
  try {
    ...
    const dom = (await base44.entities.Dom.filter({ id: dom_id }))[0];
    const aktivneNastavenia = (await base44.entities.NastavenieCenovejPonuky.filter({ aktivne: true }))[0];
...
<body>
  <div class="container">
    <div class="header">
      ...
      <h1 style="margin: 0; font-size: 28px;">Cenová ponuka - Náhľad</h1>
      <p style="margin: 5px 0 0 0; opacity: 0.9;">Model: ${dom.nazov}</p>
    </div>

    <div class="content">
      <div class="client-info">
        <h3 style="margin-top: 0; color: #374151;">Informácie o klientovi</h3>
        <p style="margin: 5px 0;"><strong>Meno:</strong> ${klient_meno || '[Meno klienta]'}</p>
        <p style="margin: 5px 0;"><strong>Email:</strong> ${klient_email || '[Email]'}</p>
        <p style="margin: 5px 0;"><strong>Telefón:</strong> ${klient_telefon || '[Telefón]'}</p>
