...
export default function KonfiguratorPH008() {
  ...
  useEffect(() => {
    if (typStavby === 'rodinny_dom') {
      const premiumInsulationIndex = HOUSE_PH008.options.insulation.length > 2 ? 2 : HOUSE_PH008.options.insulation.length - 1;
        ...
        return (
          <Section title="Hrubá stavba" icon={Hammer}>
             ...
             <div className="col-span-full mt-6">
                <h4 className="font-semibold mb-3 text-sm text-gray-500 uppercase tracking-wide">Predĺženie domu</h4>
          ...
          <Section title="Exteriér" icon={Thermometer}>
             ...
             <div className="col-span-full mt-6">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    {[
                      { l: 'Strešné', p: getPrice('addon', 'roofWindow', HOUSE_PH008.addons.roofWindow), v: roofWindows, s: setRoofWindows, k: 'roofWindow' },
                      { l: 'Fixné 90x205', p: getPrice('addon', 'fixWindow', HOUSE_PH008.addons.fixWindow), v: fixWindows, s: setFixWindows, k: 'fixWindow' },
                      { l: 'Výklopné 90x205', p: getPrice('addon', 'tiltWindowBig', HOUSE_PH008.addons.tiltWindowBig), v: tiltWindowsBig, s: setTiltWindowsBig, k: 'tiltWindowBig' },
                      { l: 'Výklopné 55x90', p: getPrice('addon', 'tiltWindowSmall', HOUSE_PH008.addons.tiltWindowSmall), v: tiltWindowsSmall, s: setTiltWindowsSmall, k: 'tiltWindowSmall' }
                    ].map((item, idx) => (
        ...
        return (
          <Section title="Interiér" icon={Layout}>
                        ...
                        <div>
                           ...
                           <div className="text-sm text-gray-500 font-medium flex items-center gap-1">
                             ...
                             {isAdmin ? (
                                <input 
                                  type="number" 
                                  value={getPrice('addon', 'interiorDoor', HOUSE_PH008.addons.interiorDoor)} 
                                ...
                                <span className="text-indigo-600 font-bold">{getPrice('addon', 'interiorDoor', HOUSE_PH008.addons.interiorDoor)} €</span>
                             )}
                           </div>
                        </div>
                    </div>
            ...
            <div className="col-span-full mt-6 grid grid-cols-1 gap-3">
                  <AddonCheckbox label="Laminácia farby okien" price={getPrice('addon', 'windowLamination', HOUSE_PH008.addons.windowLamination)} checked={windowLamination} onChange={() => setWindowLamination(!windowLamination)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'windowLamination', p)} />
                  <AddonCheckbox label="Tónované sklá" price={getPrice('addon', 'windowTint', HOUSE_PH008.addons.windowTint)} checked={windowTint} onChange={() => setWindowTint(!windowTint)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'windowTint', p)} />
                  <AddonCheckbox label="Podlahy laminát" price={getPrice('addon', 'laminateFloors', HOUSE_PH008.addons.laminateFloors)} checked={laminateFloors} onChange={() => setLaminateFloors(!laminateFloors)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'laminateFloors', p)} />
                  <AddonCheckbox label="Podlahové kúrenie" price={getPrice('addon', 'floorHeating', HOUSE_PH008.addons.floorHeating)} checked={floorHeating} onChange={() => setFloorHeating(!floorHeating)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'floorHeating', p)} />
            </div>
          </Section>
        );
      case 4:
        return (
          <Section title="Technológie" icon={Zap}>
             <div className="col-span-full grid grid-cols-1 gap-3">
                <AddonCheckbox label="Elektro rozvody" price={getPrice('addon', 'electricity', HOUSE_PH008.addons.electricity)} checked={electricity} onChange={() => setElectricity(!electricity)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'electricity', p)} />
                <AddonCheckbox label="Voda a odpady" price={getPrice('addon', 'water', HOUSE_PH008.addons.water)} checked={water} onChange={() => setWater(!water)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'water', p)} />
                <AddonCheckbox label="Sanita" price={getPrice('addon', 'sanita', HOUSE_PH008.addons.sanita)} checked={sanita} onChange={() => setSanita(!sanita)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'sanita', p)} />
                <AddonCheckbox label="Bojler" price={getPrice('addon', 'boiler', HOUSE_PH008.addons.boiler)} checked={boiler} onChange={() => setBoiler(!boiler)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'boiler', p)} />
              </div>
              <div className="col-span-full mt-8 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                 ...
                 <div className="grid grid-cols-1 gap-3">
                    <AddonCheckbox 
                      ...
                      price={getPrice('addon', 'heatPump', HOUSE_PH008.addons.heatPump)} 
                      checked={heatPump} 
                    ...
                    <AddonCheckbox 
                      label="Rekuperácia" 
                      price={getPrice('addon', 'recuperation', HOUSE_PH008.addons.recuperation)} 
      ...
      case 5:
        return (
          <Section title="Služby" icon={FileText}>
              <div className="col-span-full grid grid-cols-1 gap-3">
                <AddonCheckbox label="Projektant" price={getPrice('addon', 'projectant', HOUSE_PH008.addons.projectant)} checked={projectant} onChange={() => setProjectant(!projectant)} locked={typStavby === 'rodinny_dom'} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'projectant', p)} />
                <AddonCheckbox 
                  label="Inžiniering" 
                  ...
                  price={getPrice('addon', 'engineering', HOUSE_PH008.addons.engineering)} 
                  checked={engineering} 
                  onChange={() => setEngineering(!engineering)} 
                  isAdmin={isAdmin} 
                  onPriceChange={(p) => updatePrice('addon', 'engineering', p)} 
                />
                <AddonCheckbox label="Revízie" price={getPrice('addon', 'revision', HOUSE_PH008.addons.revision)} checked={revision} onChange={() => {}} disabled={true} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'revision', p)} />
                <AddonCheckbox label="Prípojky sietí" price={getPrice('addon', 'networks', HOUSE_PH008.addons.networks)} checked={networks} onChange={() => setNetworks(!networks)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'networks', p)} />
              </div>
    ...
    <>
      ...
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 p-4 rounded-xl border-2 border-gray-200 mb-5">
        <div className="flex justify-between items-center">
          ...
          <span className="text-xl font-black text-gray-900">{HOUSE_PH008.basePrice.toLocaleString()} €</span>
        </div>
      </div>

      <SummaryGroup title="Hrubá stavba" icon={Hammer}>
        <SummaryItem label="Montáž hrubej stavby" price={getPrice('mounting', mountingIdx, HOUSE_PH008.options.mounting[mountingIdx].price)} active={mountingIdx > 0} info={mountingIdx > 0 ? HOUSE_PH008.options.mounting[mountingIdx].label : undefined} />
        <SummaryItem label="Predĺženie domu" price={getPrice('extension', extensionIdx, HOUSE_PH008.options.extension[extensionIdx].price)} active={extensionIdx > 0} info={extensionIdx > 0 ? HOUSE_PH008.options.extension[extensionIdx].label : undefined} />
        <SummaryItem label="Izolácia" price={getPrice('insulation', insulationIdx, HOUSE_PH008.options.insulation[insulationIdx].price)} active={true} info={HOUSE_PH008.options.insulation[insulationIdx].label} />
        <SummaryItem label="Základy" price={getPrice('foundation', foundationIdx, HOUSE_PH008.options.foundation[foundationIdx].price)} active={foundationIdx > 0} info={foundationIdx > 0 ? HOUSE_PH008.options.foundation[foundationIdx].label : undefined} />
        <SummaryItem label="Vstupné dvere" price={getPrice('doors', doorsIdx, HOUSE_PH008.options.doors[doorsIdx].price)} active={true} info={HOUSE_PH008.options.doors[doorsIdx].label} />
        <SummaryItem label="Fasáda" price={getPrice('facade', facadeIdx, HOUSE_PH008.options.facade[facadeIdx].price)} active={true} info={HOUSE_PH008.options.facade[facadeIdx].label} />
      </SummaryGroup>

      <SummaryGroup title="Holodom" icon={Layout}>
        <SummaryItem label="Interiér finiš" price={getPrice('interior', interiorIdx, HOUSE_PH008.options.interior[interiorIdx].price)} active={interiorIdx > 0} info={interiorIdx > 0 ? HOUSE_PH008.options.interior[interiorIdx].label : undefined} />
        <SummaryItem label={`Interiérové dvere (${interiorDoorsCount} ks)`} price={interiorDoorsCount * getPrice('addon', 'interiorDoor', HOUSE_PH008.addons.interiorDoor)} active={interiorDoorsCount > 0} />
        <SummaryItem label="Elektrická inštalácia" price={getPrice('addon', 'electricity', HOUSE_PH008.addons.electricity)} active={electricity} />
        <SummaryItem label="Rozvody vody a kanalizácie" price={getPrice('addon', 'water', HOUSE_PH008.addons.water)} active={water} />
        <SummaryItem label="Sanita komplet" price={getPrice('addon', 'sanita', HOUSE_PH008.addons.sanita)} active={sanita} />
        <SummaryItem label="Bojler" price={getPrice('addon', 'boiler', HOUSE_PH008.addons.boiler)} active={boiler} />
        <SummaryItem label="Tepelné čerpadlo" price={getPrice('addon', 'heatPump', HOUSE_PH008.addons.heatPump)} active={heatPump} />
        <SummaryItem label="Rekuperácia" price={getPrice('addon', 'recuperation', HOUSE_PH008.addons.recuperation)} active={recuperation} />
        <SummaryItem label="Prípojky sietí" price={getPrice('addon', 'networks', HOUSE_PH008.addons.networks)} active={networks} />
        <SummaryItem label="Laminácia farby okien" price={getPrice('addon', 'windowLamination', HOUSE_PH008.addons.windowLamination)} active={windowLamination} />
        <SummaryItem label="Tónované sklá" price={getPrice('addon', 'windowTint', HOUSE_PH008.addons.windowTint)} active={windowTint} />
        { (roofWindows > 0 || fixWindows > 0 || tiltWindowsBig > 0 || tiltWindowsSmall > 0) &&
            <SummaryItem 
                label="Doplnkové okná" 
                price={
                    roofWindows * getPrice('addon', 'roofWindow', HOUSE_PH008.addons.roofWindow) + 
                    fixWindows * getPrice('addon', 'fixWindow', HOUSE_PH008.addons.fixWindow) + 
                    tiltWindowsBig * getPrice('addon', 'tiltWindowBig', HOUSE_PH008.addons.tiltWindowBig) + 
                    tiltWindowsSmall * getPrice('addon', 'tiltWindowSmall', HOUSE_PH008.addons.tiltWindowSmall)
                } 
                active={true} 
      ...
      <SummaryGroup title="Dom na kľúč" icon={Key}>
        <SummaryItem label="Podlahy - Laminát" price={getPrice('addon', 'laminateFloors', HOUSE_PH008.addons.laminateFloors)} active={laminateFloors} />
        <SummaryItem label="Elektrické podlahové vykurovanie" price={getPrice('addon', 'floorHeating', HOUSE_PH008.addons.floorHeating)} active={floorHeating} />
      </SummaryGroup>

      <SummaryGroup title="Dokumentácia" icon={FileText}>
          <SummaryItem label="Inžiniering" price={getPrice('addon', 'engineering', HOUSE_PH008.addons.engineering)} active={engineering} />
          <SummaryItem label="Projektant" price={getPrice('addon', 'projectant', HOUSE_PH008.addons.projectant)} active={projectant} />
          <SummaryItem label="Revízna dokumentácia" price={getPrice('addon', 'revision', HOUSE_PH008.addons.revision)} active={revision} />
      ...
      <div className="bg-white border-b sticky top-0 z-30 shadow-sm backdrop-blur-md bg-white/90">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="text-center">
            <h1 className="text-lg font-bold text-gray-900 leading-none">{HOUSE_PH008.name}</h1>
          ...
          <div className="mt-8">
            <div className="bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden">
              <div className="bg-gray-900 text-white p-6">
                ...
                <p className="text-gray-400 text-sm mt-1">{HOUSE_PH008.name}</p>
              </div>
