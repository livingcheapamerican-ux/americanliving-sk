             ...
             <div className="col-span-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    {[
                      { l: 'Strešné', p: getPrice('addon', 'roofWindow', HOUSE_PH006.addons.roofWindow), v: roofWindows, s: setRoofWindows, k: 'roofWindow' },
                      { l: 'Fixné 90x205', p: getPrice('addon', 'fixWindow', HOUSE_PH006.addons.fixWindow), v: fixWindows, s: setFixWindows, k: 'fixWindow' },
                      { l: 'Výklopné 90x205', p: getPrice('addon', 'tiltWindowBig', HOUSE_PH006.addons.tiltWindowBig), v: tiltWindowsBig, s: setTiltWindowsBig, k: 'tiltWindowBig' },
                      { l: 'Výklopné 55x90', p: getPrice('addon', 'tiltWindowSmall', HOUSE_PH006.addons.tiltWindowSmall), v: tiltWindowsSmall, s: setTiltWindowsSmall, k: 'tiltWindowSmall' }
                    ].map((item, idx) => (
        ...
        return (
          <Section title="Interiér" icon={Layout}>
            ...
            <div className="col-span-full mt-6 grid grid-cols-1 gap-3">
                  <AddonCheckbox label="Laminácia farby okien" price={getPrice('addon', 'windowLamination', HOUSE_PH006.addons.windowLamination)} checked={windowLamination} onChange={() => setWindowLamination(!windowLamination)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'windowLamination', p)} />
                  <AddonCheckbox label="Tónované sklá" price={getPrice('addon', 'windowTint', HOUSE_PH006.addons.windowTint)} checked={windowTint} onChange={() => setWindowTint(!windowTint)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'windowTint', p)} />
                  <AddonCheckbox label="Podlahy laminát" price={getPrice('addon', 'laminateFloors', HOUSE_PH006.addons.laminateFloors)} checked={laminateFloors} onChange={() => setLaminateFloors(!laminateFloors)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'laminateFloors', p)} />
                  <AddonCheckbox label="Podlahové kúrenie" price={getPrice('addon', 'floorHeating', HOUSE_PH006.addons.floorHeating)} checked={floorHeating} onChange={() => setFloorHeating(!floorHeating)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'floorHeating', p)} />
            </div>
          </Section>
        );
      case 4:
        return (
          <Section title="Technológie" icon={Zap}>
             <div className="col-span-full grid grid-cols-1 gap-3">
                <AddonCheckbox label="Elektro rozvody" price={getPrice('addon', 'electricity', HOUSE_PH006.addons.electricity)} checked={electricity} onChange={() => setElectricity(!electricity)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'electricity', p)} />
                <AddonCheckbox label="Voda a odpady" price={getPrice('addon', 'water', HOUSE_PH006.addons.water)} checked={water} onChange={() => setWater(!water)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'water', p)} />
                <AddonCheckbox label="Sanita" price={getPrice('addon', 'sanita', HOUSE_PH006.addons.sanita)} checked={sanita} onChange={() => setSanita(!sanita)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'sanita', p)} />
                <AddonCheckbox label="Bojler" price={getPrice('addon', 'boiler', HOUSE_PH006.addons.boiler)} checked={boiler} onChange={() => setBoiler(!boiler)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'boiler', p)} />
              </div>
      ...
      case 5:
        return (
          <Section title="Služby" icon={FileText}>
              <div className="col-span-full grid grid-cols-1 gap-3">
                <AddonCheckbox label="Projektant" price={getPrice('addon', 'projectant', HOUSE_PH006.addons.projectant)} checked={projectant} onChange={() => setProjectant(!projectant)} locked={typStavby === 'rodinny_dom'} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'projectant', p)} />
                <AddonCheckbox 
                  label="Inžiniering" 
                ...
                />
                <AddonCheckbox label="Revízie" price={getPrice('addon', 'revision', HOUSE_PH006.addons.revision)} checked={revision} onChange={() => {}} disabled={true} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'revision', p)} />
                <AddonCheckbox label="Prípojky sietí" price={getPrice('addon', 'networks', HOUSE_PH006.addons.networks)} checked={networks} onChange={() => setNetworks(!networks)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'networks', p)} />
              </div>
  ...
  const SummaryContent = () => (
    <>
      ...
      <SummaryGroup title="Hrubá stavba" icon={Hammer}>
        <SummaryItem label="Montáž hrubej stavby" price={getPrice('mounting', mountingIdx, HOUSE_PH006.options.mounting[mountingIdx].price)} active={mountingIdx > 0} info={mountingIdx > 0 ? HOUSE_PH006.options.mounting[mountingIdx].label : undefined} />
        <SummaryItem label="Izolácia" price={getPrice('insulation', insulationIdx, HOUSE_PH006.options.insulation[insulationIdx].price)} active={true} info={HOUSE_PH006.options.insulation[insulationIdx].label} />
        <SummaryItem label="Základy" price={getPrice('foundation', foundationIdx, HOUSE_PH006.options.foundation[foundationIdx].price)} active={foundationIdx > 0} info={foundationIdx > 0 ? HOUSE_PH006.options.foundation[foundationIdx].label : undefined} />
        <SummaryItem label="Vstupné dvere" price={getPrice('doors', doorsIdx, HOUSE_PH006.options.doors[doorsIdx].price)} active={true} info={HOUSE_PH006.options.doors[doorsIdx].label} />
        <SummaryItem label="Fasáda" price={getPrice('facade', facadeIdx, HOUSE_PH006.options.facade[facadeIdx].price)} active={true} info={HOUSE_PH006.options.facade[facadeIdx].label} />
      </SummaryGroup>

      <SummaryGroup title="Holodom" icon={Layout}>
        <SummaryItem label="Interiér finiš" price={getPrice('interior', interiorIdx, HOUSE_PH006.options.interior[interiorIdx].price)} active={interiorIdx > 0} info={interiorIdx > 0 ? HOUSE_PH006.options.interior[interiorIdx].label : undefined} />
        <SummaryItem label={`Interiérové dvere (${interiorDoorsCount} ks)`} price={interiorDoorsCount * getPrice('addon', 'interiorDoor', HOUSE_PH006.addons.interiorDoor)} active={interiorDoorsCount > 0} />
        <SummaryItem label="Elektrická inštalácia" price={getPrice('addon', 'electricity', HOUSE_PH006.addons.electricity)} active={electricity} />
        <SummaryItem label="Rozvody vody a kanalizácie" price={getPrice('addon', 'water', HOUSE_PH006.addons.water)} active={water} />
        <SummaryItem label="Sanita komplet" price={getPrice('addon', 'sanita', HOUSE_PH006.addons.sanita)} active={sanita} />
        <SummaryItem label="Bojler" price={getPrice('addon', 'boiler', HOUSE_PH006.addons.boiler)} active={boiler} />
        <SummaryItem label="Tepelné čerpadlo" price={getPrice('addon', 'heatPump', HOUSE_PH006.addons.heatPump)} active={heatPump} />
        <SummaryItem label="Rekuperácia" price={getPrice('addon', 'recuperation', HOUSE_PH006.addons.recuperation)} active={recuperation} />
        <SummaryItem label="Prípojky sietí" price={getPrice('addon', 'networks', HOUSE_PH006.addons.networks)} active={networks} />
        <SummaryItem label="Laminácia farby okien" price={getPrice('addon', 'windowLamination', HOUSE_PH006.addons.windowLamination)} active={windowLamination} />
        <SummaryItem label="Tónované sklá" price={getPrice('addon', 'windowTint', HOUSE_PH006.addons.windowTint)} active={windowTint} />
      ...
      <SummaryGroup title="Dom na kľúč" icon={Key}>
        <SummaryItem label="Podlahy - Laminát" price={getPrice('addon', 'laminateFloors', HOUSE_PH006.addons.laminateFloors)} active={laminateFloors} />
        <SummaryItem label="Elektrické podlahové vykurovanie" price={getPrice('addon', 'floorHeating', HOUSE_PH006.addons.floorHeating)} active={floorHeating} />
      </SummaryGroup>

      <SummaryGroup title="Dokumentácia" icon={FileText}>
          <SummaryItem label="Inžiniering" price={getPrice('addon', 'engineering', HOUSE_PH006.addons.engineering)} active={engineering} />
          <SummaryItem label="Projektant" price={getPrice('addon', 'projectant', HOUSE_PH006.addons.projectant)} active={projectant} />
          <SummaryItem label="Revízna dokumentácia" price={getPrice('addon', 'revision', HOUSE_PH006.addons.revision)} active={revision} />
