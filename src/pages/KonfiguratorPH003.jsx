    ...
    "insulation": [
      {
        ...
        "price": 0,
        "description": "Štandardná izolácia"
      },
      {
    ...
    "foundation": [
      {
        "label": "Bez základov",
        "price": 0,
        "description": "Klient si realizuje sám"
      },
      {
        "label": "Pilóty/Pätky",
        "price": 6348,
...
export default function KonfiguratorPH003() {
  ...
  const { data: domFromDb } = useQuery({
    queryKey: ['dom-ph003', domIdFromUrl],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (domFromDb?.konfigurator_custom_ceny_prosto_house?.['ph003']) {
      setCustomPrices(domFromDb.konfigurator_custom_ceny_prosto_house['ph003']);
    }
  }, [domFromDb]);
  ...
  useEffect(() => {
    if (typStavby === 'rodinny_dom') {
      const premiumInsulationIndex = HOUSE_PH003.options.insulation.length > 2 ? 2 : HOUSE_PH003.options.insulation.length - 1;
  ...
  const isA0Compliant = useMemo(() => {
    const currentInsulation = HOUSE_PH003.options.insulation[insulationIdx];
  ...
  const totalPrice = useMemo(() => {
    ...
    mountingIdx, insulationIdx, foundationIdx, interiorIdx, doorsIdx, facadeIdx,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (sanita) selectedItems.push({ name: "Sanita", price: getPrice('addon', 'sanita', HOUSE_PH003.addons.sanita), selected: true, section: "holodom" });
      if (boiler) selectedItems.push({ name: "Bojler", price: getPrice('addon', 'boiler', HOUSE_PH003.addons.boiler), selected: true, section: "holodom" });
      ...
      if (laminateFloors) selectedItems.push({ name: "Laminátové podlahy", price: getPrice('addon', 'laminateFloors', HOUSE_PH003.addons.laminateFloors), selected: true, section: "kluc" });
      if (floorHeating) selectedItems.push({ name: "Podlahové kúrenie", price: getPrice('addon', 'floorHeating', HOUSE_PH003.addons.floorHeating), selected: true, section: "kluc" });
      if (windowLamination) selectedItems.push({ name: "Laminácia okien", price: getPrice('addon', 'windowLamination', HOUSE_PH003.addons.windowLamination), selected: true, section: "holodom" });
      if (windowTint) selectedItems.push({ name: "Tónované sklá", price: getPrice('addon', 'windowTint', HOUSE_PH003.addons.windowTint), selected: true, section: "holodom" });
      if (interiorDoorsCount > 0) selectedItems.push({ name: `Interiérové dvere (${interiorDoorsCount} ks)`, price: interiorDoorsCount * getPrice('addon', 'interiorDoor', HOUSE_PH003.addons.interiorDoor), selected: true, section: "holodom" });
      if (roofWindows > 0) selectedItems.push({ name: `Strešné okná (${roofWindows} ks)`, price: roofWindows * getPrice('addon', 'roofWindow', HOUSE_PH003.addons.roofWindow), selected: true, section: "hruba" });
      if (fixWindows > 0) selectedItems.push({ name: `Fixné okná (${fixWindows} ks)`, price: fixWindows * getPrice('addon', 'fixWindow', HOUSE_PH003.addons.fixWindow), selected: true, section: "hruba" });
      if (tiltWindowsBig > 0) selectedItems.push({ name: `Výklopné okná veľké (${tiltWindowsBig} ks)`, price: tiltWindowsBig * getPrice('addon', 'tiltWindowBig', HOUSE_PH003.addons.tiltWindowBig), selected: true, section: "hruba" });
      if (tiltWindowsSmall > 0) selectedItems.push({ name: `Výklopné okná malé (${tiltWindowsSmall} ks)`, price: tiltWindowsSmall * getPrice('addon', 'tiltWindowSmall', HOUSE_PH003.addons.tiltWindowSmall), selected: true, section: "hruba" });
      if (networks) selectedItems.push({ name: "Prípojky sietí", price: getPrice('addon', 'networks', HOUSE_PH003.addons.networks), selected: true, section: "docs" });
      if (engineering) selectedItems.push({ name: "Inžiniering", price: getPrice('addon', 'engineering', HOUSE_PH003.addons.engineering), selected: true, section: "docs" });
      ...
      const response = await base44.functions.invoke('odosliCenovuPonukuProstoHouse', {
        ...
        izolaciaNavysenie: HOUSE_PH003.options.insulation[insulationIdx].label.includes('250') ? 'premium' : 'standard',
        zaklady: HOUSE_PH003.options.foundation[foundationIdx].label,
        vstupneDvere: HOUSE_PH003.options.doors[doorsIdx].label,
        ...
        interierFinis: HOUSE_PH003.options.interior[interiorIdx].label,
          ...
          <Section title={t('roughConstruction')} icon={Hammer}>
             <div className="col-span-full">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {HOUSE_PH003.options.mounting.map((opt, i) => {
             ...
             <div className="col-span-full mt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {HOUSE_PH003.options.foundation.map((opt, i) => {
        ...
        return (
          <Section title={t('stepExterior')} icon={Thermometer}>
             <div className="col-span-full">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {HOUSE_PH003.options.insulation.map((opt, i) => {
             ...
             <div className="col-span-full mt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {HOUSE_PH003.options.facade.map((opt, i) => {
              ...
              <div className="col-span-full mt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {HOUSE_PH003.options.doors.map((opt, i) => {
             ...
             <div className="col-span-full mt-6">
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    {[
                      { l: t('roofWindow'), p: getPrice('addon', 'roofWindow', HOUSE_PH003.addons.roofWindow), v: roofWindows, s: setRoofWindows, k: 'roofWindow' },
                      { l: t('fixedWindow'), p: getPrice('addon', 'fixWindow', HOUSE_PH003.addons.fixWindow), v: fixWindows, s: setFixWindows, k: 'fixWindow' },
                      { l: t('tiltWindowBig'), p: getPrice('addon', 'tiltWindowBig', HOUSE_PH003.addons.tiltWindowBig), v: tiltWindowsBig, s: setTiltWindowsBig, k: 'tiltWindowBig' },
                      { l: t('tiltWindowSmall'), p: getPrice('addon', 'tiltWindowSmall', HOUSE_PH003.addons.tiltWindowSmall), v: tiltWindowsSmall, s: setTiltWindowsSmall, k: 'tiltWindowSmall' }
                    ].map((item, idx) => (
        ...
        return (
          <Section title={t('interior')} icon={Layout}>
            <div className="col-span-full">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                   {HOUSE_PH003.options.interior.map((opt, i) => {
                        ...
                        <div>
                           ...
                           <div className="text-sm text-gray-500 font-medium flex items-center gap-1">
                             ...
                             {isAdmin ? (
                                <input 
                                  type="number" 
                                  value={getPrice('addon', 'interiorDoor', HOUSE_PH003.addons.interiorDoor)} 
                                ...
                                <span className="text-indigo-600 font-bold">{getPrice('addon', 'interiorDoor', HOUSE_PH003.addons.interiorDoor)} €</span>
                             )}
                           </div>
                        </div>
                    </div>
            ...
            <div className="col-span-full mt-6 grid grid-cols-1 gap-3">
                  <AddonCheckbox label={t('windowLamination')} price={getPrice('addon', 'windowLamination', HOUSE_PH003.addons.windowLamination)} checked={windowLamination} onChange={() => setWindowLamination(!windowLamination)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'windowLamination', p)} t={t} />
                  <AddonCheckbox label={t('tintedGlass')} price={getPrice('addon', 'windowTint', HOUSE_PH003.addons.windowTint)} checked={windowTint} onChange={() => setWindowTint(!windowTint)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'windowTint', p)} t={t} />
                  <AddonCheckbox label={t('laminateFloors')} price={getPrice('addon', 'laminateFloors', HOUSE_PH003.addons.laminateFloors)} checked={laminateFloors} onChange={() => setLaminateFloors(!laminateFloors)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'laminateFloors', p)} t={t} />
                  <AddonCheckbox label={t('floorHeating')} price={getPrice('addon', 'floorHeating', HOUSE_PH003.addons.floorHeating)} checked={floorHeating} onChange={() => setFloorHeating(!floorHeating)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'floorHeating', p)} t={t} />
            </div>
          </Section>
        );
      case 4:
        return (
          <Section title={t('technologies')} icon={Zap}>
             <div className="col-span-full grid grid-cols-1 gap-3">
                <AddonCheckbox label={t('electricalWiring')} price={getPrice('addon', 'electricity', HOUSE_PH003.addons.electricity)} checked={electricity} onChange={() => setElectricity(!electricity)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'electricity', p)} t={t} />
                <AddonCheckbox label={t('waterDrainage')} price={getPrice('addon', 'water', HOUSE_PH003.addons.water)} checked={water} onChange={() => setWater(!water)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'water', p)} t={t} />
                <AddonCheckbox label={t('sanitary')} price={getPrice('addon', 'sanita', HOUSE_PH003.addons.sanita)} checked={sanita} onChange={() => setSanita(!sanita)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'sanita', p)} t={t} />
                <AddonCheckbox label={t('boiler')} price={getPrice('addon', 'boiler', HOUSE_PH003.addons.boiler)} checked={boiler} onChange={() => setBoiler(!boiler)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'boiler', p)} t={t} />
              </div>
              <div className="col-span-full mt-8 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                 ...
                 <div className="grid grid-cols-1 gap-3">
                    <AddonCheckbox 
                      label={t('heatPump')} 
                      price={getPrice('addon', 'heatPump', HOUSE_PH003.addons.heatPump)} 
                      checked={heatPump} 
                    ...
                    <AddonCheckbox 
                      ...
                      price={getPrice('addon', 'recuperation', HOUSE_PH003.addons.recuperation)} 
      ...
      case 5:
        return (
          <Section title={t('services')} icon={FileText}>
              <div className="col-span-full grid grid-cols-1 gap-3">
                <AddonCheckbox label={t('projectant')} price={getPrice('addon', 'projectant', HOUSE_PH003.addons.projectant)} checked={projectant} onChange={() => setProjectant(!projectant)} locked={typStavby === 'rodinny_dom'} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'projectant', p)} t={t} />
                <AddonCheckbox 
                  label={t('engineering')} 
                  description={t('engineeringDesc')}
                  price={getPrice('addon', 'engineering', HOUSE_PH003.addons.engineering)} 
                  checked={engineering} 
                  ...
                  t={t}
                />
                <AddonCheckbox label={t('revisions')} price={getPrice('addon', 'revision', HOUSE_PH003.addons.revision)} checked={revision} onChange={() => {}} disabled={true} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'revision', p)} t={t} />
                <AddonCheckbox label={t('networkConnections')} price={getPrice('addon', 'networks', HOUSE_PH003.addons.networks)} checked={networks} onChange={() => setNetworks(!networks)} isAdmin={isAdmin} onPriceChange={(p) => updatePrice('addon', 'networks', p)} t={t} />
              </div>
    ...
    <>
      ...
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 p-4 rounded-xl border-2 border-gray-200 mb-5">
        <div className="flex justify-between items-center">
          ...
          <span className="text-xl font-black text-gray-900">{HOUSE_PH003.basePrice.toLocaleString()} €</span>
        </div>
      </div>

      <SummaryGroup title={t('shellConstruction')} icon={Hammer}>
        <SummaryItem label={t('assemblyItem')} price={getPrice('mounting', mountingIdx, HOUSE_PH003.options.mounting[mountingIdx].price)} active={mountingIdx > 0} />
        <SummaryItem label={t('insulationItem')} price={getPrice('insulation', insulationIdx, HOUSE_PH003.options.insulation[insulationIdx].price)} active={true} />
        <SummaryItem label={t('foundationsItem')} price={getPrice('foundation', foundationIdx, HOUSE_PH003.options.foundation[foundationIdx].price)} active={foundationIdx > 0} />
        <SummaryItem label={t('doorsItem')} price={getPrice('doors', doorsIdx, HOUSE_PH003.options.doors[doorsIdx].price)} active={true} />
        <SummaryItem label={t('facadeItem')} price={getPrice('facade', facadeIdx, HOUSE_PH003.options.facade[facadeIdx].price)} active={true} />
      </SummaryGroup>

      <SummaryGroup title={t('shellHouse')} icon={Layout}>
        <SummaryItem label={t('interiorFinishItem')} price={getPrice('interior', interiorIdx, HOUSE_PH003.options.interior[interiorIdx].price)} active={interiorIdx > 0} />
        <SummaryItem label={`${t('interiorDoorsItem')} (${interiorDoorsCount} ks)`} price={interiorDoorsCount * getPrice('addon', 'interiorDoor', HOUSE_PH003.addons.interiorDoor)} active={interiorDoorsCount > 0} />
        <SummaryItem label={t('electricalInstallation')} price={getPrice('addon', 'electricity', HOUSE_PH003.addons.electricity)} active={electricity} />
        <SummaryItem label={t('waterAndDrainage')} price={getPrice('addon', 'water', HOUSE_PH003.addons.water)} active={water} />
        <SummaryItem label={t('sanitaryComplete')} price={getPrice('addon', 'sanita', HOUSE_PH003.addons.sanita)} active={sanita} />
        <SummaryItem label={t('boilerItem')} price={getPrice('addon', 'boiler', HOUSE_PH003.addons.boiler)} active={boiler} />
        <SummaryItem label={t('heatPumpItem')} price={getPrice('addon', 'heatPump', HOUSE_PH003.addons.heatPump)} active={heatPump} />
        <SummaryItem label={t('recuperationItem')} price={getPrice('addon', 'recuperation', HOUSE_PH003.addons.recuperation)} active={recuperation} />
        <SummaryItem label={t('networkConnectionsItem')} price={getPrice('addon', 'networks', HOUSE_PH003.addons.networks)} active={networks} />
        <SummaryItem label={t('windowLaminationItem')} price={getPrice('addon', 'windowLamination', HOUSE_PH003.addons.windowLamination)} active={windowLamination} />
        <SummaryItem label={t('tintedGlassItem')} price={getPrice('addon', 'windowTint', HOUSE_PH003.addons.windowTint)} active={windowTint} />
        { (roofWindows > 0 || fixWindows > 0 || tiltWindowsBig > 0 || tiltWindowsSmall > 0) &&
            <SummaryItem 
                label={t('additionalWindowsLabel')} 
                price={
                    roofWindows * getPrice('addon', 'roofWindow', HOUSE_PH003.addons.roofWindow) + 
                    fixWindows * getPrice('addon', 'fixWindow', HOUSE_PH003.addons.fixWindow) + 
                    tiltWindowsBig * getPrice('addon', 'tiltWindowBig', HOUSE_PH003.addons.tiltWindowBig) + 
                    tiltWindowsSmall * getPrice('addon', 'tiltWindowSmall', HOUSE_PH003.addons.tiltWindowSmall)
                } 
                active={true} 
      ...
      <SummaryGroup title={t('turnkeyHouse')} icon={Key}>
        <SummaryItem label={t('laminateFloorsItem')} price={getPrice('addon', 'laminateFloors', HOUSE_PH003.addons.laminateFloors)} active={laminateFloors} />
        <SummaryItem label={t('floorHeatingItem')} price={getPrice('addon', 'floorHeating', HOUSE_PH003.addons.floorHeating)} active={floorHeating} />
      </SummaryGroup>

      <SummaryGroup title={t('documentation')} icon={FileText}>
          <SummaryItem label={t('engineeringItem')} price={getPrice('addon', 'engineering', HOUSE_PH003.addons.engineering)} active={engineering} />
          <SummaryItem label={t('projectantItem')} price={getPrice('addon', 'projectant', HOUSE_PH003.addons.projectant)} active={projectant} />
          <SummaryItem label={t('revisionsItem')} price={getPrice('addon', 'revision', HOUSE_PH003.addons.revision)} active={revision} />
        ...
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
              <span className="text-xs text-gray-500">{STEPS[activeStep].title} ({activeStep + 1}/{STEPS.length})</span>
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden">
              <div className="bg-gradient-to-r from-indigo-900 via-purple-900 to-pink-900 text-white p-8 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-indigo-200 text-sm mt-2">{HOUSE_PH003.name}</p>
                </div>
              </div>
