...
export default function Domov() {
  ...
  return (
    <div className="warm min-h-screen -mt-10 sm:-mt-12 md:-mt-14 lg:-mt-16 xl:-mt-20 overflow-x-hidden lg:overflow-x-visible relative text-[#2C3A33]">
      ...
      {/* Sekcia 5: AI Architekt KEXO */}
      <section className="py-12 sm:py-16 relative overflow-hidden">
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-5xl mx-auto">
            <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 border border-[#C5A880]/30 rounded-3xl p-6 sm:p-10 lg:p-12 shadow-2xl relative overflow-hidden text-white flex flex-col lg:flex-row items-center gap-8 sm:gap-12">
              
              {/* Ambient Background Glow */}
              <div className="absolute top-0 right-0 w-96 h-96 bg-[#C5A880]/10 rounded-full blur-3xl pointer-events-none" />
              <div className="absolute bottom-0 left-0 w-80 h-80 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none" />

              {/* Left Column: Information & Presets */}
              <div className="flex-1 text-left relative z-10">
                <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#C5A880]/20 border border-[#C5A880]/40 text-xs sm:text-sm font-black uppercase tracking-wider text-[#E2C799] mb-4">
                  ...
                  <span>AI ARCHITEKT & PORADCA KEXO</span>
                </div>
                <h2 className="text-2xl sm:text-4xl font-extrabold text-white mb-4 leading-tight tracking-tight">
                  Pýtajte sa <span className="text-[#E2C799]">AI Architekta KEXO</span> na čokoľvek
                </h2>
                <p className="text-sm sm:text-base text-slate-300 leading-relaxed font-normal mb-6">
                  Získajte okamžité odpovede na stavebné povolenia, pozemky, Dotáciu AMERICANA a 100% financovanie. KEXO pre vás vypočíta presný odhad ceny aj mesačnej úspory energie do 10 sekúnd.
                </p>

                {/* Quick Question Pills */}
                <div className="space-y-2.5 mb-8">
                  <button
                    onClick={() => window.dispatchEvent(new CustomEvent('openChatbotWithContext', {
                      detail: { message: "Mám nárok na súkromnú Dotáciu AMERICANA až do 15 000 €?" }
                    }))}
                    className="w-full text-left bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl p-3 text-xs sm:text-sm text-slate-200 hover:text-white transition-all flex items-center justify-between group"
                  >
                    <span>💬 "Mám nárok na súkromnú Dotáciu AMERICANA až do 15 000 €?"</span>
                    <ArrowRight className="w-4 h-4 text-[#C5A880] group-hover:translate-x-1 transition-transform" />
                  </button>
                  <button
                    onClick={() => window.dispatchEvent(new CustomEvent('openChatbotWithContext', {
                      detail: { message: "Ako funguje 100% financovanie stavby bez vlastnej hotovosti?" }
                    }))}
                    className="w-full text-left bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl p-3 text-xs sm:text-sm text-slate-200 hover:text-white transition-all flex items-center justify-between group"
                  >
                    <span>💬 "Ako funguje 100% financovanie stavby bez vlastných úspor?"</span>
                    <ArrowRight className="w-4 h-4 text-[#C5A880] group-hover:translate-x-1 transition-transform" />
                  </button>
                </div>
                ...
                <button 
                  ...
                  className="w-full sm:w-auto bg-gradient-to-r from-[#C5A880] to-[#E2C799] hover:from-[#d5b890] hover:to-[#f2d7a9] text-slate-950 font-black px-8 py-4 rounded-xl shadow-lg hover:shadow-[#C5A880]/30 transition-all text-sm sm:text-base flex items-center justify-center gap-2"
                >
                  <MessageCircle className="w-5 h-5 text-slate-950" />
                  <span>Spustiť rozhovor s KEXO AI</span>
                  <ArrowRight className="w-5 h-5" />
                </button>
              </div>

              {/* Right Column: Interactive Chat Card Preview */}
              <div className="w-full lg:w-96 shrink-0 relative z-10">
                <div className="bg-slate-900/90 border border-[#C5A880]/30 rounded-2xl p-5 shadow-2xl relative overflow-hidden backdrop-blur-md">
                  {/* Top Header */}
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-[#C5A880]/20 border border-[#C5A880]/40 flex items-center justify-center">
                        <Sparkles className="w-5 h-5 text-[#E2C799]" />
                      </div>
                      <div>
                        <p className="text-sm font-bold text-white leading-tight">KEXO AI Architekt</p>
                        <p className="text-xs text-emerald-400 flex items-center gap-1.5">
                          <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></span>
                          Online • Pripravený odpovedať
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Sample Stream */}
                  <div className="space-y-3 mb-4 min-h-[140px] flex flex-col justify-end">
                    <div className="bg-slate-800/80 border border-slate-700 rounded-2xl p-3 text-xs text-slate-200 leading-relaxed">
                      👋 Dobrý deň! Som váš AI Architekt KEXO. Rád vám pomôžem s výberom modulárneho domu, Dotáciou AMERICANA aj hypotékou. Na čo sa chcete opýtať?
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <input 
                      type="text" 
                      placeholder="Napíšte otázku pre KEXO AI..."
                      className="flex-1 bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-[#C5A880]"
                      readOnly
                      onClick={() => window.dispatchEvent(new CustomEvent('openChatbot'))}
                    />
                    <button 
                      onClick={() => window.dispatchEvent(new CustomEvent('openChatbot'))}
                      className="bg-[#C5A880] hover:bg-[#E2C799] text-slate-950 p-3 rounded-xl transition-colors shrink-0"
                    >
