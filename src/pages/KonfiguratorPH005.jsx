...
export default function KonfiguratorPH005() {
  ...
  const { data: domFromDb } = useQuery({
    queryKey: ['dom-ph005', domIdFromUrl],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (domFromDb?.konfigurator_custom_ceny_prosto_house?.['ph005']) {
      setCustomPrices(domFromDb.konfigurator_custom_ceny_prosto_house['ph005']);
    }
  }, [domFromDb]);
  ...
  useEffect(() => {
    if (typStavby === 'rodinny_dom') {
      const premiumInsulationIndex = HOUSE_PH005.options.insulation.length > 2 ? 2 : HOUSE_PH005.options.insulation.length - 1;
  ...
  const isA0Compliant = useMemo(() => {
    const currentInsulation = HOUSE_PH005.options.insulation[insulationIdx];
        ...
        return (
          <Section title={t('interior')} icon={Layout}>
            <div className="col-span-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {HOUSE_PH005.options.interior.map((opt, i) => (
                    <ConfiguratorTile 
                      key={i} 
                      label={opt.label} 
                        ...
                        <div>
                           ...
                           <div className="text-sm text-gray-500 font-medium flex items-center gap-1">
                             ...
                             {isAdmin ? (
                                <input 
                                  type="number" 
                                  value={getPrice('addon', 'interiorDoor', HOUSE_PH005.addons.interiorDoor)} 
                                ...
                                <span className="text-indigo-600 font-bold">{getPrice('addon', 'interiorDoor', HOUSE_PH005.addons.interiorDoor)} €</span>
                             )}
                           </div>
                        </div>
                    </div>
        ...
        { (roofWindows > 0 || fixWindows > 0 || tiltWindowsBig > 0 || tiltWindowsSmall > 0) &&
            <SummaryItem 
                ...
                price={
                    ...
                    tiltWindowsBig * getPrice('addon', 'tiltWindowBig', HOUSE_PH005.addons.tiltWindowBig) + 
                    tiltWindowsSmall * getPrice('addon', 'tiltWindowSmall', HOUSE_PH005.addons.tiltWindowSmall)
                } 
                active={true} 
      ...
      <div className="bg-white border-b sticky top-0 z-30 shadow-sm backdrop-blur-md bg-white/90">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
          <div className="flex-1 min-w-0">
            <h1 className="text-sm font-bold text-gray-900 leading-tight">{HOUSE_PH005.name}</h1>
