  ...
  if (isA0Compliant) {
    return (
      <div className="mt-3 p-3 bg-green-50 rounded-xl border border-green-200">
        ...
        <p className="text-[11px] text-green-600 mt-1 ml-6">{t ? t('meetsA0CertDesc') : 'Vhodné na kolaudáciu ako rodinný dom.'}</p>
      </div>
    );
  }
      ...
      <div className="space-y-1.5 ml-6">
        {missing.map((m, i) => (
          <button
            ...
            className="flex items-center gap-2 text-xs text-amber-800 hover:text-amber-900 group w-full text-left"
          >
