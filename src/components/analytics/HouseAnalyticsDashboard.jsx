  ...
  const houseStats = useMemo(() => {
    ...
    sessions.forEach(session => {
      ...
      session.dom_interactions.forEach(interaction => {
        ...
        // Odhad veku podľa času návštevy a správania
        const hour = new Date(session.start_time).getHours();
        let ageGroup = '35-44'; // default
        if (hour >= 0 && hour < 6) ageGroup = '18-24';
        else if (hour >= 6 && hour < 9) ageGroup = '35-44';
        else if (hour >= 9 && hour < 17 && session.device_info?.device_type === 'mobile') ageGroup = '25-34';
        ...
        stat.visitors.push({
          ...
          duration: interaction.duration_seconds || 0,
          device: deviceType,
          city: city || 'N/A'
        });
      });
