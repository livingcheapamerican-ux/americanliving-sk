      ...
      <div className="relative flex-1 min-w-0">
        <div className="flex items-center gap-1 sm:gap-2 mb-0.5">
          <motion.span 
            ...
            className="inline-flex items-center justify-center px-1 sm:px-2 py-0.5 bg-white/30 backdrop-blur-sm rounded-full text-white text-[8px] sm:text-xs font-bold uppercase tracking-wider shadow-sm"
          >
            {t('phase')} {step}
          </motion.span>
        </div>
        <h3 className="text-xs sm:text-lg font-bold text-white tracking-tight truncate drop-shadow-md">{title}</h3>
        {subtitle && <p className="text-white/90 text-[9px] sm:text-xs mt-0.5 truncate drop-shadow">{subtitle}</p>}
      </div>
    </motion.div>
  );
