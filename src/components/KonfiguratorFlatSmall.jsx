...
export default function KonfiguratorFlatSmall({ 
  ...
  return (
    <div className="mt-8 relative">
      {showTypSelector && typStavby === "" && (
        <motion.div 
          key="typ-selector"
          initial={{ opacity: 0, y: -20 }} 
          animate={{ opacity: 1, y: 0 }} 
          className="min-h-[70vh] flex items-center justify-center px-2 sm:px-4 w-full max-w-full overflow-hidden mb-8"
        >
          <div className="max-w-4xl w-full overflow-hidden">
            ...
            >
              <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-2xl p-8 mb-8 shadow-xl">
                ...
                <h2 className="text-3xl font-bold mb-2">{t('createYourPriceOffer')}</h2>
                <div className="flex items-center justify-center gap-2 text-purple-100">
                  <CheckCircle className="w-4 h-4" />
                  <span className="text-sm">{t('noHiddenFees')}</span>
                </div>
              </div>
              <h3 className="text-2xl font-bold text-gray-800 mb-3">{t('buildingTypeQuestion')}</h3>
              <p className="text-gray-600 max-w-2xl mx-auto mb-4">{t('buildingTypeDesc')}</p>
              <motion.div 
                className="bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-xl p-4 max-w-lg mx-auto shadow-lg"
                animate={{ scale: [1, 1.02, 1], boxShadow: ["0 10px 25px rgba(59, 130, 246, 0.3)", "0 15px 35px rgba(139, 92, 246, 0.4)", "0 10px 25px rgba(59, 130, 246, 0.3)"] }}
                transition={{ duration: 2, repeat: Infinity }}
              >
                <div className="flex items-center justify-center gap-3">
                  <motion.div animate={{ y: [0, -5, 0] }} transition={{ duration: 1.5, repeat: Infinity }}>👇</motion.div>
                  <p className="text-sm sm:text-base font-bold">{t('buildingTypeQuestion')}</p>
                  <motion.div animate={{ y: [0, -5, 0] }} transition={{ duration: 1.5, repeat: Infinity, delay: 0.2 }}>👇</motion.div>
                </div>
              </motion.div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
              <motion.div
                ...
                onClick={() => handleTypStavbyChange("rekreacna")}
                className="cursor-pointer w-full relative"
              >
                <motion.div className="absolute inset-0 rounded-lg" animate={{ boxShadow: ["0 0 0 0 rgba(251, 191, 36, 0)", "0 0 0 8px rgba(251, 191, 36, 0.1)", "0 0 0 0 rgba(251, 191, 36, 0)"] }} transition={{ duration: 2, repeat: Infinity }} />
                <Card className={`p-3 sm:p-6 h-full transition-all border-2 w-full relative ${typStavby === "rekreacna" ? "border-amber-500 bg-amber-50 shadow-xl ring-2 ring-amber-300" : "border-gray-200 hover:border-amber-300 hover:shadow-lg"}`}>
                  {!typStavby && (
                    <motion.div className="absolute -top-3 -right-3 bg-amber-500 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg z-10" animate={{ scale: [1, 1.1, 1], rotate: [-5, 5, -5] }} transition={{ duration: 1, repeat: Infinity }}>
                      {t('clickHere')} 👆
                    </motion.div>
                  )}
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div>
                      <h4 className="text-xl font-bold text-gray-800 mb-1">{t('recreationalBuilding')}</h4>
                      <Badge className="bg-amber-500 text-white text-xs">{t('economicChoice')}</Badge>
                    </div>
                  </div>
                  <ul className="space-y-2 text-sm text-gray-700">
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-amber-600 mt-0.5 flex-shrink-0" /><span>{t('cottage')}</span></li>
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-amber-600 mt-0.5 flex-shrink-0" /><span>{t('yearRoundInsulation')}</span></li>
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-amber-600 mt-0.5 flex-shrink-0" /><span>{t('noEnergyCertificate')}</span></li>
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-amber-600 mt-0.5 flex-shrink-0" /><span>{t('lowerPrice')}</span></li>
                  </ul>
                  <div className="mt-6 pt-4 border-t border-amber-200"><p className="text-xs text-gray-500 italic">{t('meetsRecreationalParams')}</p></div>
                </Card>
              </motion.div>
              ...
              <motion.div
                ...
                whileHover={{ scale: 1.02, y: -4 }}
                onClick={() => handleTypStavbyChange("rodinny_dom")}
                className="cursor-pointer w-full relative"
              >
                <motion.div className="absolute inset-0 rounded-lg" animate={{ boxShadow: ["0 0 0 0 rgba(34, 197, 94, 0)", "0 0 0 8px rgba(34, 197, 94, 0.1)", "0 0 0 0 rgba(34, 197, 94, 0)"] }} transition={{ duration: 2, repeat: Infinity, delay: 0.5 }} />
                <Card className={`p-3 sm:p-6 h-full transition-all border-2 w-full relative ${typStavby === "rodinny_dom" ? "border-green-500 bg-green-50 shadow-xl ring-2 ring-green-300" : "border-gray-200 hover:border-green-300 hover:shadow-lg"}`}>
                  {!typStavby && (
                    <motion.div className="absolute -top-3 -right-3 bg-green-500 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg z-10" animate={{ scale: [1, 1.1, 1], rotate: [5, -5, 5] }} transition={{ duration: 1, repeat: Infinity, delay: 0.3 }}>
                      {t('clickHere')} 👆
                    </motion.div>
                  )}
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div>
                      <h4 className="text-xl font-bold text-gray-800 mb-1">{t('familyHouseA0')}</h4>
                      <Badge className="bg-gradient-to-r from-green-500 to-emerald-600 text-white text-xs">⚡ {t('recommended')}</Badge>
                    </div>
                  </div>
                  <ul className="space-y-2 text-sm text-gray-700">
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" /><span>{t('yearRoundLiving')}</span></li>
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" /><span>{t('energyCertificateA0')}</span></li>
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" /><span>{t('premiumInsulation250300')}</span></li>
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" /><span>{t('heatPumpRecuperation')}</span></li>
                    <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-green-600 mt-0.5 flex-shrink-0" /><span>{t('permanentResidencePossibility')}</span></li>
                  </ul>
                  <div className="mt-6 pt-4 border-t border-green-200"><p className="text-xs text-gray-500 italic">{t('meetsAllStandardsFamilyHouse')}</p></div>
                </Card>
              </motion.div>
            </div>

            {typStavby && (
              <motion.div key="services" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-8">
                <Card className="p-6 border-2 border-cyan-300 shadow-lg bg-gradient-to-b from-cyan-50/50 to-white">
                  ...
                  <div className="space-y-3">
                    <motion.div whileHover={{ scale: 1.01 }} onClick={() => setPredajNehnutelnosti?.(!predajNehnutelnosti)} className={`p-4 rounded-lg cursor-pointer transition-all border-2 ${predajNehnutelnosti ? 'bg-blue-100 border-blue-500 shadow-md' : 'bg-white border-gray-200 hover:border-blue-300'}`}>
                      <div className="flex items-start gap-3">{predajNehnutelnosti ? <CheckCircle className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" /> : <Square className="w-5 h-5 text-gray-400 mt-0.5 flex-shrink-0" />}<div className="flex-1"><p className="font-semibold text-gray-800">{t('sellPreviousProperty')}</p><p className="text-xs text-gray-600 mt-1">{t('sellPreviousPropertyDesc')}</p></div></div>
                    </motion.div>
                    <motion.div whileHover={{ scale: 1.01 }} onClick={() => setHladaniePozemku?.(!hladaniePozemku)} className={`p-4 rounded-lg cursor-pointer transition-all border-2 ${hladaniePozemku ? 'bg-green-100 border-green-500 shadow-md' : 'bg-white border-gray-200 hover:border-green-300'}`}>
                      <div className="flex items-start gap-3">{hladaniePozemku ? <CheckCircle className="w-5 h-5 text-green-600 mt-0.5 flex-shrink-0" /> : <Square className="w-5 h-5 text-gray-400 mt-0.5 flex-shrink-0" />}<div className="flex-1"><p className="font-semibold text-gray-800">{t('wantLandForHouse')}</p><p className="text-xs text-gray-600 mt-1">{t('wantLandForHouseDesc')}</p></div></div>
                    </motion.div>
                    <motion.div whileHover={{ scale: 1.01 }} onClick={() => setFinancneSluzby?.(!financneSluzby)} className={`p-4 rounded-lg cursor-pointer transition-all border-2 ${financneSluzby ? 'bg-amber-100 border-amber-500 shadow-md' : 'bg-white border-gray-200 hover:border-amber-300'}`}>
                      <div className="flex items-start gap-3">{financneSluzby ? <CheckCircle className="w-5 h-5 text-amber-600 mt-0.5 flex-shrink-0" /> : <Square className="w-5 h-5 text-gray-400 mt-0.5 flex-shrink-0" />}<div className="flex-1"><p className="font-semibold text-gray-800">{t('financialServicesLoans')}</p><p className="text-xs text-gray-600 mt-1">{t('financialServicesLoansDesc')}</p></div></div>
                    </motion.div>
                  </div>
                </Card>
                <div className="text-center mt-6">
                  <Button onClick={() => {}} className="bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white font-bold px-12 py-4 rounded-xl shadow-lg hover:shadow-xl transition-all">
                    {t('continue')}
                  </Button>
                </div>
              </motion.div>
                ...
                <div className="relative p-3 sm:p-5 md:p-6">
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 sm:gap-8">
                    <div className="flex-1">
                      ...
                      <h3 className="text-base sm:text-xl font-bold text-gray-900 mb-1">{dom?.nazov || 'Flat Small 41m²'}</h3>
                  ...
                  <div className="mt-4 sm:mt-6 pt-3 sm:pt-5 border-t border-slate-200 flex justify-start">
                    <Button 
                      size="lg" 
                      onClick={() => setShowContactModal(true)}
                      className="bg-gradient-to-r from-green-500 via-emerald-500 to-teal-500 hover:from-green-600 hover:via-emerald-600 hover:to-teal-600 text-white font-bold text-xs sm:text-sm px-4 sm:px-8 py-3 sm:py-4 shadow-2xl shadow-green-500/30 transition-all hover:scale-105 hover:shadow-green-500/40"
                    >
