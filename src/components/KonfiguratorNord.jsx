...
import { 
  ...
  Square, FileCheck, Package, Hammer, Key, Sparkles, CheckCircle, Building2
} from "lucide-react";
...
export default function KonfiguratorNord({ 
  ...
  const BASE_PRICE = dom?.zakladna_cena || 54900;

  const { animations, triggerAnimation } = useFlyingAnimation();
  const { t } = useLanguage();

  // NORD CENNÍK - FIXNÉ CENY (64m²)
  const CENY = {
    montaz: { nie: 0, ano: 9225 },
    dvere: { ziadne: 0, kovove: 720, plastove: 660 },
    izolacia: { standard: 0, zvysena: 2200, premium: 4400, ultra: 8250 },
    elektroinstalacia: 3200,
    vodaKanalizacia: 950,
    sanitaKomplet: 1169,
    bojler: 264,
    tepelneCerpadlo: 2721,
    rekuperacia: 1300,
    zaklady: { bez: 0, skrutky: 3900, doska: 7900, pasove: 9700 },
    pripojkaSiete: 1501,
    inziniering: 2592,
    projektA0: 3500,
    interierFinis: { ziadne: 0, drevo: 6700, sadrokarton: 7700 },
    vonkajsiaFasada: { standard: 0, suchana: 5200 },
    povrchokaOkien: 1200,
    vnutornePodlahy: 1440,
    podlahovVykurovanie: 3240,
    pergola: 0,
  ...
  const a0Odporucania = useMemo(() => {
    ...
    const chybajuce = [];
    if (izolaciaNavysenie !== "premium") chybajuce.push("Premium izolácia (200mm steny, 250mm strecha)");
  ...
  const selectedItems = useMemo(() => {
    ...
    const izolaciaLabel = izolaciaNavysenie === "premium" ? t('insulationPremium') + " (200/250mm)" : izolaciaNavysenie === "zvysena" ? t('insulationEnhanced') + " (150/200mm)" : t('insulationStd');
