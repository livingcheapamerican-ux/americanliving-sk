...
function LayoutContent({ children }) {
  ...
  const navItems = [
    ...
    { name: t('about') || "O nás", path: createPageUrl("ONas"), icon: Info },
    { name: t('contact') || "Kontakt", path: createPageUrl("Kontakt"), icon: Phone },
    ...(isAdmin ? [{ name: '💰 ' + (t('adminPriceList') || "Cenník Admin"), path: createPageUrl("AdminCennik"), icon: Grid3x3 }] : []),
    ...(isAdmin || user?.super_admin ? [{ name: '👤 ' + (t('myAccount') || "Moje Konto"), path: '/MojeKonto', icon: Users, isMyAccount: true }] : []),
  ];
          ...
          <div className="flex items-center justify-between gap-1 sm:gap-2">
              ...
              {[...navItems, ...adminNavItems].map((item) => {
                ...
                return (
                  <Link
                    ...
                    to={item.path}
                    data-tour={tourId}
                    className={`rounded-full font-bold transition-all px-3 py-1.5 xl:px-4 xl:py-2 text-xs xl:text-sm whitespace-nowrap ${
                      isActive(item.path)
            ...
            <div className="hidden sm:flex items-center gap-2">
              ...
              <a 
                ...
                className="hidden xl:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-100/80 dark:bg-slate-800/80 border border-slate-200 dark:border-white/10 text-xs font-bold text-slate-800 dark:text-slate-200 hover:text-red-600 dark:hover:text-red-400 shrink-0 transition-all shadow-sm"
              >
                <Phone className="w-3.5 h-3.5 text-red-600 animate-pulse" />
                <span>+421 905 138 124</span>
              </a>

              {/* Download Catalog Primary CTA */}
              <Link to={createPageUrl("StiahniteSiNasKatalog")} className="hidden md:inline-flex shrink-0">
                <Button className="rounded-full bg-red-600 hover:bg-red-700 text-white text-xs lg:text-sm font-bold px-4 py-2 shadow-md hover:shadow-red-600/30 transition-all flex items-center gap-1.5">
              ...
              <button
                onClick={toggleTheme}
                type="button"
                className="flex items-center justify-center p-2 rounded-full border border-slate-200 dark:border-white/10 bg-slate-100/50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200 hover:bg-[#C5A880]/15 hover:text-[#C5A880] dark:hover:text-[#C5A880] transition-all duration-300 h-9 w-9"
