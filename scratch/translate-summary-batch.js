...
async function translateBatch(objToTranslate, targetLangName) {
  const prompt = `Translate the following JSON object containing UI labels from Slovak to ${targetLangName}. Keep the same style, tone, format, and structure. Do NOT translate brand names like Ticabhouse, ProstoHouse, Barnhouse, A-FRAME, or American Living. Return ONLY a valid JSON object as response, no other text or explanation. JSON to translate:\n\n${JSON.stringify(objToTranslate, null, 2)}`;
  
  const responseText = await callInternalLLM(prompt);
...
async function run() {
  ...
  for (const lang of targetLanguages) {
    ...
    const toTranslateBatch = {};
    for (const [key, slovakValue] of Object.entries(summaryUIStrings)) {
      if (!obj[key]) {
        toTranslateBatch[key] = slovakValue;
      }
    }

    const missingKeysCount = Object.keys(toTranslateBatch).length;
    if (missingKeysCount > 0) {
      console.log(`  Translating ${missingKeysCount} missing keys to ${lang.name}...`);
      try {
        const translatedBatch = await translateBatch(toTranslateBatch, lang.name);
        for (const [key, translatedVal] of Object.entries(translatedBatch)) {
          obj[key] = translatedVal;
        }
        fs.writeFileSync(langPath, 'export default ' + JSON.stringify(obj, null, 2) + ';\n', 'utf8');
        console.log(`  ✓ Saved ${lang.code}.js with translated keys`);
      } catch (e) {
        console.error(`  ✗ Error translating batch: ${e.message}. Falling back to individual keys.`);
        // Fallback to one-by-one translation if JSON parsing fails
        for (const [key, slovakValue] of Object.entries(toTranslateBatch)) {
          try {
            const prompt = `Translate this UI label from Slovak to ${lang.name}. Keep the same style, tone, format, and structure. Return ONLY the translated text, nothing else. Text:\n\n${slovakValue}`;
            const translated = await callInternalLLM(prompt);
            obj[key] = translated;
