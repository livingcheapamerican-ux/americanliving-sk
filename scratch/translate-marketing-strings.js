...
async function run() {
  ...
  // 1. Add keys to sk.js directly
  const skPath = path.join(localesDir, 'sk.js');
  let skContent = fs.readFileSync(skPath, 'utf8');
  const skLastBrace = skContent.lastIndexOf('}');
  
  let skAppend = ',\n';
  let skAdded = 0;
  for (const [key, value] of Object.entries(marketingStrings)) {
    if (!skContent.includes(`"${key}":`)) {
      skAppend += `  "${key}": ${JSON.stringify(value)},\n`;
      skAdded++;
    }
  }
  skAppend = skAppend.replace(/,\n$/, '\n');
  
  if (skAdded > 0) {
    skContent = skContent.substring(0, skLastBrace) + skAppend + skContent.substring(skLastBrace);
    fs.writeFileSync(skPath, skContent, 'utf8');
    console.log(`✓ Updated sk.js (${skAdded} keys)`);
  } else {
    console.log("⊙ sk.js already updated");
  }

  // 2. Translate and add keys to other languages
  for (const lang of targetLanguages) {
    const langPath = path.join(localesDir, `${lang.code}.js`);
    let langContent = fs.readFileSync(langPath, 'utf8');
    const langLastBrace = langContent.lastIndexOf('}');
    ...
    for (const [key, value] of Object.entries(marketingStrings)) {
      if (!langContent.includes(`"${key}":`)) {
        ...
        if (navTranslations[lang.code] && navTranslations[lang.code][key]) {
          ...
          try {
            translated = await translateText(value, lang.name);
          } catch (e) {
            console.error(`Error translating: ${e.message}`);
            translated = value; // Fallback
          }
        }
        langAppend += `  "${key}": ${JSON.stringify(translated)},\n`;
        addedCount++;
        await new Promise(resolve => setTimeout(resolve, 300)); // small delay
      }
    }
    
    langAppend = langAppend.replace(/,\n$/, '\n');
    
    if (addedCount > 0) {
      langContent = langContent.substring(0, langLastBrace) + langAppend + langContent.substring(langLastBrace);
      fs.writeFileSync(langPath, langContent, 'utf8');
      console.log(`✓ Updated ${lang.code}.js (${addedCount} keys)`);
    } else {
      console.log(`⊙ ${lang.code}.js already has all keys`);
    }
  }

  console.log("\nAll translations successfully added to locales!");
}
